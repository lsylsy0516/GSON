...
class Detection_Remover:
    def __init__(self) -> None:
        ...
        self.map_path = "/home/orin/dashgo_ws/li10_2.pgm"
        # self.map_path = "/home/orin/dashgo_ws/map.pgm"
        ...
        rospy.loginfo(f"Costmap shape:{self.costmap.shape}")
        ...
        if self.map_flag:
            ...
            for pose in msg.poses:
                ...
                if x >= costmap.shape[1] or y >= costmap.shape[0] :
                    rospy.loginfo("out of order,escape")
                    rospy.loginfo(f"current_xy:({x},{y})")
                    continue


                if costmap[y,x] < 10 :
                    cv2.circle(costmap,(x,y),6*2,200,-1)  # 说明这个边缘误识别
                elif costmap[y,x]>100:
                    cv2.circle(costmap,(x,y),10*2,100,-1)
