...
# 这个函数是用来计算每个点到最近的一个检测框的距离，如果距离小于检测框的半径，就认为这个点是属于这个检测框的
def scans_to_cutout(
    scans,
    scan_phi,
    stride=1,
    ...
    inds_ct_low = _clip(np.floor(inds_ct), 0, num_pts - 1).astype(np.int64)
    inds_ct_high = _clip(inds_ct_low + 1, 0, num_pts - 1).astype(np.int64)
