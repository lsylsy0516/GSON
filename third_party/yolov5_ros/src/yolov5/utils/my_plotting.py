...
class Annotator:
    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False):
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                w, h = cv2.getTextSize(label, 0, fontScale=self.sf*2, thickness=self.tf*2)[0]  # text width, height
                h += 3  # add pixels to pad text
                outside = p1[1] >= h  # label fits outside box
                
                # 将标签的位置调整到正上方
                center_x = p1[0] + (p2[0] - p1[0]) // 2  # 边界框的中心x位置
                p1 = (center_x - w // 2, p1[1] - h if outside else p1[1] + h)  # 调整标签位置，居中对齐
                if p1[0] < 0:  # 防止标签超出左边界
                    p1 = (0, p1[1])
                if p1[0] + w > self.im.shape[1]:  # 防止标签超出右边界
                    p1 = (self.im.shape[1] - w, p1[1])
                
                p2 = p1[0] + w, p1[1] + h  # 标签的下边框位置
                cv2.rectangle(self.im, p1, p2, color, -1, cv2.LINE_AA)  # filled
                cv2.putText(
                    self.im,
                    label,
                    (p1[0], p1[1] + h - 1),  # 根据新的位置调整文本
                    0,
                    self.sf*2,
                    txt_color,
                    thickness=self.tf*2,
